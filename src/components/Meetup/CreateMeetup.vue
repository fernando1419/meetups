      ...
      <v-layout row>
         <v-flex xs12>
            <v-form @submit.prevent="onCreateMeetup">
               <v-layout row>
                  <v-flex xs12 sm6 offset-sm3>
                     <v-text-field id="title" v-model="title" name="title" label="Title" required />
                  </v-flex>
               </v-layout>
               <v-layout row>
                  <v-flex xs12 sm6 offset-sm3>
                     <v-text-field id="location" v-model="location" name="location" label="Location" required />
                  </v-flex>
               </v-layout>
               <v-layout row>
                  <v-flex xs12 sm6 offset-sm3>
                     <v-file-input v-model="image" label="File input" dense append-icon="mdi-camera" accept="image/*"
                                   small-chips
                                   show-size
                                   loading="true"
                                   @change="onFilePicked"
               ...
               <v-layout v-show="imageUrl" row>
                  <v-flex xs12 sm6 offset-sm3>
                     <img :src="imageUrl" height="140px" width="200px" />
                  </v-flex>
               </v-layout>
               <v-layout row>
                  <v-flex xs12 sm6 offset-sm3>
                     <v-textarea v-model="description" name="description" label="Description" hint="Tell us about the meetup" rows="3" />
                  </v-flex>
               </v-layout>
